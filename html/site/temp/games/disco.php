<!DOCTYPE html> 
<html lang="nl">
   <head>
       <meta charset="UTF-8">
       <meta http-equiv="X-UA-Compatible" content="IE=edge">
       <meta name="viewport" content="width=device-width, initial-scale=1.0">
       <meta name="author" content="B. de Haan & A. Panagiotidis">
       <meta name="description" content="een game revieuw sites gemaakt door gamers voor gamers">
       <meta name="keywords" content="revieuws, games, gamestars, gs, sale, summer sale, merch, rating sites, sandbox, disco, disco elysium">
       <title>GameStars</title>
       <script src="../scripts/main.js" defer></script>
       <link rel="stylesheet" href="../css/style.css">
    <link rel="icon" href="../img/logoTransperent.png">
    </head>
<body>
    <header>
        <nav class="logo">
            <a href="../index.php"><img id="logo" src="../img/logo.png" alt="logo GameStars"></a>
        </nav>
        <h1 id="logonaam">GameStars</h1>
        <nav class="topnav">
            <a href="../index.php">Home</a>
            <a href="../merch.php">Merch</a>   
            <a class="active" href="../games.php">Games</a>    
            <a href="../aboutUs.php">About Us & Contact Us</a>
        </nav>
    </header>
    <main>

    <section class="container">
            <article class="product-container">
                <img src="../img/Disco_elysium.png" alt="disco elysium">
                    <article class="product-details">
                        <h2>Disco Elysium</h2>
                        <h3><b>€19,95</b></h3>
                        <p><b>Genre:</b> [sandbox]</p>
                        <p><b>Rating:</b>⭐⭐⭐⭐</p>
                        <p><b>Beschrijving</b></p>
                        <p> Disco Elysium is een verhaalgedreven RPG waarin spelers de rol aannemen van een detective met een uniek vaardigheidssysteem tot hun beschikking. Het spel richt zich meer op het vertellen van verhalen, dialoog en karakterontwikkeling dan op gevechten. Het wordt geprezen om zijn diepgaande verhaal, complexe personages en innovatieve mechanics.</p>
                    </article>
            </article>
        </section>
        <button onclick="buy()" class="buy">
            Koop
        </button>
    </main>

    <footer>
        <article>
            <h6 class="disclaimer">disclaimer: de redactie is niet verantwoordelijk voor fouten in de informatie</h6>
        </article>
    </footer>
    
</body>

</html>